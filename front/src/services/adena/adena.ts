...
export class AdenaService {
	...
	static async establishConnection(name: string): Promise<void> {
		...
		// Parse the response
		if (response.status === EAdenaResponseStatus.SUCCESS || response.type === EAdenaResponseType.ALREADY_CONNECTED) {
	...
	static async sendTransaction(messages: IAdenaMessage[], gasWanted: number): Promise<BroadcastTxCommitResult> {
		AdenaService.validateAdena();
